...
const AffiliationFormListAdmin = () => {
  ...
  const [searchFilteredAffiliations, setSearchFilteredAffiliations] = useState([]);
  ...
  const handleFilteredDataChange = useCallback((filteredData) => {
    setSearchFilteredAffiliations(filteredData);
    setCurrentPage(1); // Reset to first page when filters change
  }, []);

  // Apply date range filtering
  useEffect(() => {
    let filtered = [...searchFilteredAffiliations];

    if (startDate || endDate) {
      filtered = filtered.filter((affiliation) => {
        if (!affiliation.createdAt) return false;
        
        const submissionDate = new Date(affiliation.createdAt);
    ...
    setCurrentPage(1);
  }, [searchFilteredAffiliations, startDate, endDate]);
  ...
  const fetchAffiliations = async () => {
    try {
      ...
      if (data.success) {
        ...
        setSearchFilteredAffiliations(data.data || []);
      } else {
    ...
    () =>
      [
        ...new Set(
          affiliations
            .map((affiliation) =>
              affiliation.jamathArea && affiliation.jamathArea[0]
                ? affiliation.jamathArea[0].district
                : null
            )
            .filter(Boolean)
        ),
  ...
  const searchFields = useMemo(
    () => [
      ...
      "affiliationNumber",
      "trackingId",
      "jamathArea.0.district",
      "jamathArea.0.area",
    ],
    []
  );
  const filterFields = useMemo(() => ["status", "jamathArea.0.district"], []);
  const uniqueFieldValues = useMemo(
    () => ({
      "jamathArea.0.district": uniqueDistricts,
    }),
    [uniqueDistricts]
  );
  const filterFieldLabels = useMemo(
    () => ({
      status: "Status",
      "jamathArea.0.district": "District",
    }),
    []
  );
                      ...
                      >
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="text-sm text-gray-600">
                            {affiliation.jamathArea && affiliation.jamathArea[0] ? 
                              `${affiliation.jamathArea[0].district || ''}, ${affiliation.jamathArea[0].area || ''}`.replace(/^,\s*|,\s*$/g, '').trim() || 'Not specified' : 
                              'Not specified'}
                          </div>
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
